...
The trip from blueprint to silicon was not a pleasure cruise for the 80386. It started at a side project for a small team in San Jose while select employees in Portland worked on the flagshit P7\footnote{a.k.a Intel i960.}, a CPU using a new instruction set capable of running high level language and memory garbage collection in hardware. When the Portland team hit a wall due to performances, the 386 went from step child to king\footnote{Intel 386 Microprocessor Design and Development Oral History Panel}. Two choices in the design of the 386 constributed to its success. For one the designers had decided to listen to the programmers feedback and dropped the idea to use a new instruction set. As a result the 386 is fully backward compatibility with the 286. They also managed to add a 32 bit operating mode solving a lot of the memory addressing issues of the 286.\\
\par
...
\par
The 80386 relies on six units with a three stages pipeline.\\
\par
\begin{figure}[H]
\centering
...
\end{figure}
\par
The Bus Unit was capable of operating with a 32 bit or a 16 bit bus. This proved a huge assest for manufacturers who were able to reuse the design of their 286 (16 bit) motherboards.
\par
\begin{figure}[H]
\centering
...
\end{figure}
\par
The three units in the execution group form a three stage pipeline where a 16 byte instruction prefetch queue feeds variable size instructions\footnote{All Intel CPUs are Complex instruction set computing.} to a three stages pipeline made of Decode, Execute, and Retire. 
...
\end{figure}
\par
\textbf{\underline{Trivia :}} The prefetcher is linear, it cannot predict the result of a branch and as a result a jump (\cw{JMP}) instruction triggers a flush in the prefetch queue and the decoded instruction FIFO.\\
\par
...
\end{itemize}
 \par
 \textbf{\underline{Trivia :}} Despite its pipeline design, the 386 cannot do an operation in less than one cycle. Even simple \cw{ADD reg, reg} or even \cw{INC reg} takes two instructions.\\
 \par
...
\end{itemize}
  \par
  For example, writing at 0xABF13 in RAM would result in writing in bank 3 at offset 0xBF1. Since this is all hardwired in the Chain-4, this extra work is fast and totally invisible to the user.
 \begin{figure}[H]
\centering
