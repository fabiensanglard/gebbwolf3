...
\par
\bu{Trivia :} 640KiB was all a game could have for executable code. But people writing compilers got clever. Strike Commander (a famous flight arcade game released in 1993) executable is 745kB, which obviously doesn't fit in Conventional Memory. The trick was to use a technique of "overlays" pages where only a few overlays are loaded when the game starts but the compiler had inserted instructions able to detect when the CPU IP\footnote{Instruction Pointer} is about to reach the end of an overlay. These instructions swapped in RAM the rest of the instructions and issued a \cw{jmp} for the CPU to follow. The technique can be seen as an exercise of graph paging.
