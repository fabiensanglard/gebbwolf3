...
\bu{Trivia :} There was no GPU market back then. Since all VGA cards had to be VGA compatible and have 256KB of RAM, most people just bought the cheapeast thing available. Note however that some cards only had a 8 bits ISA bus connector (like the trident below) and some had an ISA 16 bits connector (twice as fast).\\
\par

\begin{figure}[H] 
  \centering \includegraphics[width=\textwidth]{imgs/hardware/suntra_trident_tvga8800br.png} 
  \caption{A Trident 8800  (8 bits ISA). Photo courtesy of \cw{old.vgamuseum.info}.}
\end{figure}
\par
\begin{figure}[H] 
  \centering 
  \includegraphics[width=\textwidth]{imgs/hardware/diamond_stealth_vram_revb2.png} 
  \caption{A Diamond Stealth (16 bits ISA). Photo courtesy of \cw{old.vgamuseum.info}.}
\end{figure}
...
\par
With two buffers (a.k.a double buffering) the CPU can start writing in the second framebuffer without messing with the framebuffer being scanned to the screen\footnote{Now the CPU is capped by the CRT refresh rate. Triple buffering can solve this at the price of frame latency.}. No more tearing!
