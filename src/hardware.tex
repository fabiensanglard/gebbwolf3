...
\par
The Southbridge run on ISA\footnote{Industry Standard Architecture}. Invented in 1981, it offers a 16 bits data bus at 8.33MHz for a 66.7 Mbit/s\footnote{https://en.wikipedia.org/wiki/List\_of\_device\_bit\_rates} bandwidth . Quick math: 66.7 Mbit/s = 66,700,000 bits per seconds which is 117,857 bytes per frame. If you take of 65,000 for the VGA, that leaves 52 KB of data for controls and sounds (at 23kHz sampling a digitized sound effect consums 23,000/70 = 328 bytes per frame). Not a limiting factor but not exactly exempt from generating bottleneck if you factor in delays due to interrupts and that the CPU is not always transmitting (halving bandwidth).\\
