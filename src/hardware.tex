...
\par
Opposite page, courtesy of \cw{vgamuseum.info}, the VGA card Trident 8800 (8 bit ISA). Notice the eight chips on the left of the card forming the VRAM where the framebuffers are stored\protect\footnotemark. On bottom, the VGA card Diamond Stealth (16 bit ISA).
\pagebreak
...
To write to the VRAM, the RAM's 1MiB address space maps 64KiB starting as indicated in table \ref{vga-modes-available}. In mode \cw{13h} for example, the VRAM is mapped from 0xA0000 to 0xAFFFF.\\
...
\par
 ...
 The first mode commonly considered for game programming is mode \cw{12h}. It offers a resolution of 640x480 at 70hz with 16 colors. Each pixel is encoded in 4 bits (a nibble) spread across the four banks.\\
\par
...
\bu{Trivia :} Notice on the very left of the card in black there is an IDE Data interface to connect a CD-ROM. That was the only way to connect a CD-ROM to a PC back then.
  ...
  \begin{figure}[H] 
    \centering 
    \scaledimage{0.9}{hardware/ss.png} 
