...
\begin{document}
To study the IBM PC it is easier to first break it down in small parts. Five sub-systems form a pipepline: Inputs, CPU, RAM, Video, and Audio.\\
\begin{figure}[H]
\centering
...
\subsection{Overview}
  The ubiquitous CPU manufacturer was Intel with its line of x86 microprocessors\footnote{73\% of PC sold in 1989 featured an Intel CPU according to Intel Corporation Annual Report.}. Machines based on the 16-bit 80286 processor did not sell well but their successors with the 80386 introduced in 1985 were immensely successful\footnote{An estimated 3.74 million 386 based systems were sold in 1990. Source: Dataquest PC Magazine February 1992.}. As a result, Wolfenstein 3D was designed to run on a PC with a 386 CPU, with degraded performance (yet still playable) on a 286.\\
\par
...
\centering
 ...
 \par
  ...
  \par
 Intel built two versions of the 386: The 386-SX and 386-DX. They are identical processors yet the DX version is almost twice as powerful as the SX (on the chart the \cw{386 SX 33Mhz} and the \cw{386 DX 33Mhz} are respectively at 5.1 and 8 MIPS). This is due to a data bus between the CPU and the RAM being twice as wide on the DX (32-bit vs 16-bit). Despite its inferiority, the SX sold well because it was cheaper and a lot of people had no idea what a "bus" was, they just wanted "a 386".\\
...
\end{itemize}
\item 32 bit address bus for up to 4GB of flat addressable RAM
...
\caption{Intel 1991 ad for a "Math CoProcessor".}
\label{fig:fp_internals}
\end{figure}
...
Out of the original 1024KB, only 640KiB (called Conventional Memory) were accessible to a program. 384KiB were reserved for the UMA and every single driver installed (\codeword{.SYS} and \codeword{.COM})  took away from the remaining 640KB.
...
\end{figure}
\par
Above, a VGA card Trident 8800 (8 bits ISA). Courtesy of \cw{vgamuseum.info}. Notice the eight chip of 32 KiB (for a total of 256KiB) on the right of the card where the framebuffers are stored.
\par
\begin{figure}[H] 
  \centering 
...
\textbf{Michael Abrash - Graphic Programming Black Book}
 ...
 \\
\par
The first problem with this design is that it is unintuitive. There is no linear framebuffer and figuring out which byte correspondes to which pixel on screen is difficult.\\
\par
...
The side effect of this convenience mechanism is that 75\% of the RAM is wasted (since only 14 bits are usable for offset).\\
  ...
  \label{clearvga}
  \par
  Mode 13h looks a bit better than 12h but is in fact also terrible for games or even static images:\\
  \begin{itemize}
\item With Chain-4 all the RAM address space is used and there is no way to have a double buffer.
\item Since the resolution is 320x200, the aspect ratio (1.6) does not fit the monitor (which is 1.333). As a result the framebuffer stored in the VRAM is stretched when transfered to the CRT. It may seem like a small distortion but can be quite problematic. An example with a circle in the framebuffer displaying as an ellipse on the screen speaks for itself.
\end{itemize}
...
\bu{Trivia :} The CRT monitor was connected to the VGA card via a DE15 port. More than 20 years later manufacturers are still trying to get ride of the "VGA port" as it is now commmonly called. It looked a lot like a serial port and newcomvers often damaged it by forcing a mouse into it.
 \begin{figure}[H]
\centering
