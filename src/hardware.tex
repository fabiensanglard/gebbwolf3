...
\centering
  \begin{tikzpicture}[font=\small]
   ...
   \end{tikzpicture}
   \caption{The 386-SX and 386-DX were identical processor. Yet the DX version is twice as powerful as the SX thanks to a data bus between the CPU and the RAM twice as wide (32 bits vs 16 bits). The SX still sold well because it was cheaper to manufacture. Also a lot of people had no idea what that 16 bits/32 bits lingo meant, they just wanted "a 386".}
 \end{figure}
 \par
...
A 386 can be summarized to the following elements:
\begin{itemize}
...
\par 
The least problematic is about the language. Since C was invented on a flat memory machine, it had to be augmented by compiler manufacturers for PCs. That is how the \codeword{near} and \codeword{far} keywords came to existence. To build pointers, a set of macro are provided, respectively \codeword{FP\_SEG} and \codeword{FP\_OFF}. That's an ugly wart on the beautiful C but that's manageable. And of course \cw{libc} is also "different": \cw{malloc} returns a \cw{near} pointer and therefor can only allocate 64K. To get more than 64K, \cw{farmalloc} is needed.\\
\\
...
You also have to be careful about pointer arithmetic. A \cw{far} pointer increment only increment the offset, not the segment. If you iterate on an array more than 64KB you will end up wrapping around! You could use yet an other type of pointer \cw{int huge*} to make pointer arithmetic work beyond 64K but really, nobody wants to go there.
...
The first adapter of the family was released in 1981. The Monochrome Display
   Adapter (\codeword{MDA}) offers two colors allowing text only with 80 columns by 25 lines.  Not great, but it had the merit to be standard on every PC. Many other systems followed over the years. Each of them preserving backward compatibility.
\bigskip
...
\\
But the real reason was physical limitations and the need for a minimum bandwidth. A CRT running at 70Hz and displaying 320x240 needed a pixel every 1/(320*240*70)th of a second. Since the VGA is palette based, one pixel is one byte [0-255] translated to a 888 RGB color. So that divides bandwidth by three. But you still need one byte each 186ns but unfortunately RAM access time was 200ns. Not fast enough\footnote{Computer Graphic: Principles and Practice 2nd Edition, page 168} to refresh the screen at 70hz. If latency could not be reduced, the throughput could still be improved by accessing four banks at the time. The memory banks read in parallel had an amortized latency of 200/4 = 50ns/pixel which was fast enough.
...
\par
 The way it works is: To write four pixels next to each others on a line, you have to write one pixel in each bank. Each of these banks are mapped at the same HMA memory address. The layout is better explained with a drawing.\\
\par
\begin{figure}[H]
\centering
...
\par
To configure this mess of planes and circuitry, the four controllers are set via 300 poorly documented internal registers. Needless to say few programmers got to dive in the internal of this thing. The first drawing of the VGA was actually deceiving. The IBM VGA Reference attempted to clarify the architecture with a drawing but it only illustrated the complexity of the interactions between each controller.\\
 \begin{figure}[H]
\centering
...
To write to the VRAM the 1MB address space maps 64KB starting as indicated in Mode table. e.g: In Mode 13h, a developer have from 0xA0000 to 0xAFFFF available to access the Video RAM (VRAM). The first question that may pop in your mind is "How does one write to 256KB of RAM with only 64KB of address space?". The answer is: bank switching. Write and Read operations are routed based on a mask register indicating which bank should be read or written.\\
\par
 \begin{figure}[H]
\centering
...
When writing or reading a value V at address K in RAM, the Chain-4 breaks down that address in two parts:
\begin{itemize}
...
\end{itemize}
  \par
  For example, writing at 0xABF13 in RAM would result in writing in bank 3 at offset 0xBF1. Since this is all hardwired in the Chain-4, this address gymnastic it fast and totally transparent to the user.
 \begin{figure}[H]
\centering
...
The side effect of this convenience mechanism is that 75\% of the RAM is wasted (since only 14 bits are usable for offset).\\
  ...
  \par
  The \codeword{int 10} is a software interrupt caught by the BIOS routine in charge of Graphic setup. It looks up the \codeword{ax} register to setup all 300 VGA register with the corresponding mode. After the VGA is initialized one can write to the mapped 0xA000. It is simpler to follow with a code sample, here is some code to clear the screen to black.\\
...
\end{itemize}
\par
Even though adoption was increasing (Creative would go on selling 1M SoundBlaster in 1991), a fast majority of PCs had no sound cards which once again was a huge problem for game developers.
...
\bu{Trivia :} Notice on the very left of the card in black an IDE Data interface to connect a CD-ROM. That was the only way to connect a CD-ROM to a PC back then.
  ...
  Staring in 1990, Disney was selling an interesting piece of hardware. It was a beautiful hack. Plugged into the printer port (parallel port) of the PC, a 8 bits DAC based on the "Covox Speech Thing" was connected to a nice speaker box. It was incredibly easy to setup, simple to program and very cheap compared to the other audio solution (\$14). It would have made programmers and customers happy if it was not for a serious issue was due to the bandwidth of the port (150 kbit/s), which only allowed a 7000Hz sampling rate to be played.
  \par
  \begin{figure}[H] 
    \centering 
...
\par
The Southbridge run on ISA\footnote{Industry Standard Architecture}. Invented in 1981, it can be either:
\begin{itemize}
