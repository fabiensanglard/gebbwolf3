...
\centering
 ...
 \par
  \textbf{\underline{Trivia :}} A 2010 Core i7 Extreme Edition 980X 3,330 MHz operates at 147,600 MIPS.\\
  \par
 ...
 \par
\textbf{\underline{Trivia :}} To help meet the demand, Intel contracted AMD to manufacture 8088, 8086 and 80286 CPUs. Lawsuits, spanning over multiple decades, ensued over the i386. The mileage varied with "clones". AMD produced the 
fastest 386 (Am386DX 40MHz). Cyrix's reputation was shaky amongst gamers due to a borderline scam 486SLC (a 386SX with 486 instructions and a small L1 cache) and a 6x86 that performed terribly under Quake.\\
\par
...
\par
 As David Goldberg famously wrote, \emph{"Floating-point arithmetic is considered an esoteric subject by many people"}\footnote{"What every Computer Scientist should know about Floating-Point" by David Goldberg.}. I could not agree more. Yet it is important to understand in order to fully grasp how useful it is for programming a 3D engine. In the C language, \codeword{float}s are 32-bit container following the IEEE 754 standard. Their purpose is to store and allow operations on approximation of real numbers. The 32 bits are divided in three sections:\\
\begin{itemize}
...
\textbf{\underline{Trivia :}}  As of 2017, more than forty five years after the introduction of the 8086, in the name of backward compatibility, all PCs in the world still start in real mode. A bootloader switches them to protected mode, loads the kernel, and then actual startup can begin.
...
\textbf{\underline{Trivia :}} How big and heavy could a CRT be? The InterView 28hd96 by Intergraph had a 28" diagonal allowing a resolution of 1920x1080 at a time where most monitors displayed 640x480. It weighed 45kg (99.5lb). For comparison, a modern DELL LCD 27" weighs 7.8kg (17lb).\\
\par
...
\bu{Trivia :} Canadian companies, and especially those from Quebec, were prevalent in the early '90s due to their technological prowess. AdLib manufactured Sound Cards, Matrox made a killing with its Millenium Graphics Card, and Watcom sold the best DOS C compiler\footnote{Watcom's compiler was so good id would use it to compile Doom.}. ATI\footnote{History would repeat itself in the late '90s in the field of graphic cards: Nvidia vs ATI.} would later emerge as a major GPU innovator in the 2000s.\\
  ...
  The Sound Blaster (code named "Killer Kard"), was released in 1989 by Creative obviously targeting AdLib's dominant position.
  Two major revisions, v1.5 in 1990 and v2.0 in 1991, respectively dropped unpopular features (C/MS chips) and improved the DSP.\\ 
\par

\begin{figure}[H] 
  \centering 
  \fullimage{hardware/sb.png} 
  \caption{A SoundBlaster (v2.0). }
  \label{asb12}
\end{figure}
\par
Not only the SoundBlaster was equipped with the same OPL2 chip, providing 100\% compatibility with AdLib music playback, it was also technologically superior with a DSP\footnote{An Intel MCS-51 "Digital Sound Processor", not "Digital Signal Processor".}  allowing PCM playback (digitized sounds) at 8 bits per sample and up to 22.05kHz sampling rate. The card also came with a DA-15 port allowing joystick connection (but no MPU-401). Most importantly, the SoundBlaster was \$90 cheaper than the AdLib.\\
\par
...
Notice on the very left of the card in black there is a Panasonic interface to connect a CD-ROM. That was the only way to connect a CD-ROM to a PC back then.
  ...
  \end{figure}
\par
It was incredibly easy to set up, simple to program (it could only play one type of PCM and had no FM synthesizer), and very cheap compared to the other audio solutions (\$14). It would have made programmers and customers happy if not for one serious limitation. The parallel port bandwidth\footnote{The parallel port maximum bandwidth was 150 kbits/s at the time. Enhanced Parallel Port and later Enhanced Capability Port significantly increased the transfer rate necessary for scanners and laser printers.} allowed a sampling rate up to 18,750 Hz but the design of the DSS limited the PCM sampling rate to 7,000Hz. This was still enough to produce pleasant sounds, but fell short when compared to the 22kHz or even 44kHz of a Sound Blaster Pro.
...
The keyboard was usually plugged via a DIN-5 connector.
\begin{figure}[H]
\centering
\includegraphics[width=0.1\textwidth]{imgs/drawings/ports/DIN-5.eps}
\caption{DIN-5 Port}
\label{fig:din5Port}
\end{figure}

The PS/2 ports (a.k.a "mini DIN-6"), which became popular thanks to IBM PS/1 line in the mid-90s, allowed either a keyboard (purple port) or a mouse (green port).
 \begin{figure}[H]
\centering
