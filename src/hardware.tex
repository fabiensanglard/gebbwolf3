...
The first adapter of the family was released in 1981. The Monochrome Display
   Adapter (\codeword{MDA}) offers two colors with low resolutions but it had the merit to be on every PC sold. Many other systems followed over the years. Each of them preserving backward compatibility.
\bigskip
...
To write to the VRAM the 1MB address space maps 64KB starting as indicated in table \ref{fig:vga_modes}. A developer had from 0xA0000 to 0xAFFFF available. The first question that may pop in your mind is "How does one write to 256KB of RAM with only 64KB of address space?". The answer is: bank switching. Write and Read operation are routed based on a mask register indicating which bank should be read or written.\\
\par
 \begin{figure}[H]
\centering
...
The side effect is that 75\% of the RAM is wasted (since only 14 bits were usable for offset).\\
  ...
  \par
  The \codeword{int 10} is a software interrupt call to the BIOS routine in charge of Graphic setup. It looks up the \codeword{ax} register to setup all 300 VGA register with the corresponding mode. After the VGA is initialized one can write to the mapped 0xA000. It is simpler to follow with a code sample, here is some code to clear the screen to black:\\
...
Even though adoption was improving, a fast majority of PCs had no sound cards which once again was a huge problem for game developers.
  ...
  \subsection{AdLib}
  Started in 1988 by Martin Prevel, a former professor of music from Quebec. After an initial struggle to get game developers to use their card, Adlib convinced Sierra On-Line to support their hardware for their last title: King's Quest IV\footnote{King's Quest IV would go on selling 3 million copies.}. Soon, all game developers embraced the Adlib.\\
  \par
  ...
  \begin{figure}[H] 
    ...
    \label{Notice the Yamaha YM3812 a.k.a the OPL2 chip and the 8 bits ISA bus connector at the bottom.}
  \end{figure}
...
\begin{figure}[H] 
  ...
  \label{Notice the 8 bits ISA bus connector at the bottom.}
\end{figure}
\par
