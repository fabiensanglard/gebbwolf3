...
\cfullimage{386_layout_drawing.png}{Intel 80386-DX die layout}
\par
Featured on this double page, a 386-DX 16 Mhz. If you are holding a physical 9.25''x7.5'' copy of this book, the CPU packaging is 36.8x36.8 mm square and the die is 12x12 mm, both 1:1 scale.\\
\par
Despite the apparent complexity, the 80386 can be summarized as three systems and a three stage instruction pipeline.
...
The side effect of this convenience mechanism is that 75\% of the RAM is wasted (since only 14 bits are usable for offset).\\
  ...
  \par
  The \codeword{int 10} instruction is a software interrupt caught by the BIOS routine in charge of graphic setup. It looks up the \codeword{ax} register to setup all 300 VGA registers with the corresponding mode.\\
