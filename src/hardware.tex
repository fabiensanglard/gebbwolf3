...
\begin{document}
PC performances in the 90s were so overwhelmingly dominated by their CPU that they were simply referred to not by their brand or GPU\footnote{There were no GPU yet.} but by their Intel chip. If your PC had an Intel 80386SX in it, it was a 386. If it had an Intel 80286, it was a 286. Wolfenstein was target at 386s with degraded performances (yet still playable) on 286s.\\
\\
...
\\
This is not a problem when everything is hardwired...but it is a big problem for the 286 and the 386: \emph{They do NOT not have a hardware Floating Point Unit}. If found in the code, these operations are emulated in software by the compiler, resulting in terribly slow processing. So slow they are not usable for anything real-time. All 286 and 386 are really good at is crunching \cw{int}s which is not what a 3D game needs.\\ 
\par
...
\par
\begin{itemize}
  \item The new Protected Mode with  32 bits registers and 24 bits address bus wide offering up to 16 MB of flat RAM protectable with a MMU\footnote{Memory management Unit}.
  \item The backward compatible Real Mode. Available \emph{only} to allow old software to run. This memory model replicated how the Intel 8086 CPU from the 70s operated: 16 bits registers, 20 bits address bus giving 1MB addressable RAM with an awful segmented addressing system. 
\end{itemize}
...
\begin{Verbatim}[fontsize=\relsize{-1}]
    0000 0000 0000 0000       Segment, 16 bits shifted 4 bits left  
...
\begin{Verbatim}[fontsize=\relsize{-1}]
    0000 0000 0001 0000       Segment, 16 bits shifted 4 bits left  
...
\begin{Verbatim}[fontsize=\relsize{-1}]
    0000 0000 0001 0010       Segment, 16 bits shifted 4 bits left  
...
\bu{Trivia :} You also have to be careful about pointer arithmetic e.g: A \cw{far} pointer increment only increment the offset, not the segment. If you iterate on an array more than 64KB you can end up wrapping around! You could use \cw{huge} pointer to make pointer arithmetic work but...nobody wants to go there.
...
PCs were connected to CRT monitors: Big, heavy, small diagonal, cathodic ray based with curved surface screens. Most monitors had a 13" diagonal with a 4:3 aspect ratio. Figure \ref{fig:int_layout} shows a comparison between a 13" CRT from 1992 and a 28" LCD display from 2014.\\
