...
\end{figure}
 ...
 A 12 byte instruction prefetch queue\footnote{Documentation states 18 bytes but a bug workaround reduced the size to 12 bytes} feeds variable size instructions\footnote{All Intel CPUs are Complex instruction set computing.} to a three stages pipeline made of Decode, Execute, and Retire. Note that an instruction cannot enter the pipeline as long as the previous one as not been retired. As a result, even a simple instruction such as \cw{ADD} takes two cycles to complete\footnote{Source: http://zsmith.co/intel\_a.html\#add}.
