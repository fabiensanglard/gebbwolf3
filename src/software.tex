...
For a screen 320 pixels wide, a naive algorithm would cast 320 rays. But the engine has a trick to reduce the amount of work. It is based on XXX.\\
The following code is doing it\\
The idea is to\\
This trick alone saves X\% rays per frames. On the demo, XX frames are draw, XX rays should have been cast but only XX did.\\
Precalculated angle per pixel width (pixelangle). Precalculated sin and cos table () same thing for tangent: sintable, costable, finetangent. Granularity is one degree \#define ANGLES 360\\
tilehit encodes not only the coordinate of the hit but also the type of tile (8::8)\\
All object 64x64 are scaled and centered vertically on the screen.
