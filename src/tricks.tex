...
\end{figure}
A fast path is provided (which ironically only works if the CPU is as slow as the VGA processor) to update the palette via one \cw{lobsb} instruction. Otherwise, if not supported, a loop of 768 \cw{outsb} is used.\\
\par
