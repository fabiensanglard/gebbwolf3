...
Before drawing each column of a sprite, the engine determine if it is occluded by a wall. This is called clipping. This is an easy step than to wall and sprites being in the same 64x64 coordinate system. Once the actor's position is transformed in player space, the distance Y allows to generate a height. These heights are the same calculated for the walls. Hence, to find out if a sprite column if behind a wall, the engine just compare the height of the wall in that screen space column against the height of the actor.\\
\par
