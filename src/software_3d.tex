...
TODO.\\
\note{Ask John Carmack and John Romero how it is done. And how much Tom had to push to get his pushwalls. This is especially surprising since Catacomb-3D had secret walls.}
...
The tile array allows to build a list of visible sprites. The way the visible tile array is used is not the most subtle part of the engine (it runs in $\mathcal{O}(n^2)$ time). All sprites on the map are tested (for belonging to a visible tile) and their height is calculated. They are all added to an array unsorted. A final double loop draw all sprites far to near by comparing height.\\
\par
\begin{minipage}{\textwidth}
...
\par
Even though sprites are drawn in the same 64x64 space as wall textures, they almost never use all of it. They are usually smaller than 64 pixels and often feature transparent parts. Some even have "holes" and are composed of sequences of pixels separated by transparent areas. It is the case of the lamp which for most columns has an upper segment (the lamp) and a lower segment (the shadow).As a result sprites are stored in compressed format, discarding transparent data.\\
...
\par
There are 64 patch location per scaler (one for each length of pixels besides 64 desired), allowing to write a \cw{RETF} instruction causing an early return. The instruction to be overwritten by \cw{RETF} is saved, the scaler is unpatched once the portion of sprite column has been rendered.\\
\par
Note of the engine knows where to patch the code thanks to a cw{table location}] which is generated when the compiled shaders are.
\par
\begin{minipage}{\textwidth}
  \lstinputlisting[language={[x86masm]Assembler}]{code/self_modifying.asm}
\end{minipage}
\par
Patching the scaler is not enough. It only allows to draw a segment smaller than 64 pixels. In order to draw a "hole" properly, the engine also needs to know how many pixels skip vertically in screenspace before starting the scaler on the next segment. This is were the \cw{width} property is used. The scaler compiler also saved 64 entries to convert sprites transparent height into screen space height. This operation is only a cheap lookup.\\
\par
...
\par
\par
Sprites are stored as array of 64 columns (because of the VGA, everything is drawn vertically). Each column entry uses RLE to code what is visible. Transparent sections are completely discarded.\\
\par
