...
\end{figure}
\par
This constraint enable a variant of the DDA Algorithm\footnote{Digital differential analyzer}: A fast and accurate way to detect where a ray hits a wall on the map.

One could argue that a rasterizer approach would make more sense: With intersection easy to find, project the walls in screen space. That would avoid casting rays.\\
\begin{fancyquotes}
\par
...
Once the walls are rendered, it is time to render sprites such as enemies and decoration (lamps, table, ammos, weapons, ...). The raycaster and column renderer both provide assistance to make sure things are draw properly and does two things:
 \begin{enumerate}
  \item While casting each ray, memorize which cells were visited by the ray. This is used build a list of "things" at least partially visible.
  \item Upon drawing a wall column, save the height of that wall at this X coordinate. This is done to clip "things" and hide what is behind a wall.
 \end{enumerate}
...
Note that the engine does not place things vertically. Like walls, they are also always centered vertically. The artists cleverly drew the sprites in the same space as the wall: Not centered vertically but at the bottom of each images. This allows for a non-expensive perspective effect even though all sprites are vertically aligned.
\par
\begin{figure}[H]
 \centering
