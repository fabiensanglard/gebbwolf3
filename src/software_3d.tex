...
In "path" mode, an agent simulates patrolling. There is no path finding, it is all done via waypoints which change the direction of an agent. After stepping on a "change direction" tile, the agent keeps on walking straight until it hits a wall. All waypoints are placed manually by the map designer with TED5. In E1M1, German shepherd (marked a "D") spawn in a kennel with many waypoints.\\
\par
\begin{figure}[H]
 \centering
