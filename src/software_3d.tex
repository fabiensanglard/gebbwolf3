...
\bu{Tick :} Images are stored woven. All bytes for bank 0 first, then all bytes for bank 1 and so on. This clever way to store data allow a bank to be loaded with one single \cw{memcpy} (four per image).\\
\par
...
While it is faster to write four pixel in two operations, the trick does not provide a full 300\% speed increase since all writes have to be done in all three buffers. It is vivid when looking at the routine updating the statuc (hero's face).\\
\par
\begin{minipage}{\textwidth}
...
With the CPU \cw{float}/\cw{int} problem out of the way, it is almost time to study how a ray is cast. The coordinate system find its origin in the upper left.
\begin{figure}[H]
  \centering
 \includegraphics[width=.5\textwidth]{imgs/drawings/coordinate_system.png}
\end{figure}
\par
A map is a grid of 64 by 64 blocks. Since one block is 8 feet, maps can be 512 feet wide and tall. Fixed point variables are used all over the place. Column height are 29:3. Position is 24:8. Angle however is an \cw{int} representing 10th of degrees with a range [0, 3600].
...
\par
This part of the code relies heavily on unit circle principles. Because we need to know by how much we go up when advancing on X axis and how much we go left when advancing on the Y axis, the \cw{tan} is expecially useful. It is easier to understand with the unit circle drawn (remember that the circle radius R is equal to 1).
\begin{figure}[H]
\centering
...
Before proceeding to the next session about what is done with the distance player <-> wall, a short reminder of something taught in highschool and is the fundation of most calculations in the engine: SOH-CAH-TOA.\\
\par
 \begin{fancyquotes}
...
With a heigh calculated for a pixel, all remaining to do it "just" drawing a column of pixels.\\
...
\par
 ...
 \par 
 The way the engine does it is by removing the genericity of the scaler and generate at runtime x86 instructions specialized in drawing specific height of wall ranging from 1 pixels to 200 (the height of the screen). This is all done in \cw{BuildCompScale}.\\
\par
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{code/compscale.c}
\end{minipage}
\par
Given a height, \cw{BuildCompScale} will generate instructions in the out variable \cw{code}. In essence instead of having one generic function accepting any height,  Wolfenstein decides to have 38 functions with hard-coded height which allows to unroll the loop inside each of them.\\
\par
...
\par
What is the cost of the precompiling these scalers? How much RAM does it use? It takes 1 + 7 instructions per pixel written to scale a column. So if all size were generated from 2,4,6...152 (38 pairs of size), we can use Gauss method to calculate the size:\\
\par
$154*38*7=40,964$ bytes\\
\par
That cost was deemed too high, so past size 76 only every other size are generated (2,4,6,,..,72,74,76) and (78,82,86,...,144,148,152). RAM consumption becomes (still with Gauss method):\\
\par
...
The array allow to build a list of visible sprites. It is not the most subtle part of the engine since all sprites on the map are tested and sorted far to near but it works:\\
\par
\begin{minipage}{\textwidth}
