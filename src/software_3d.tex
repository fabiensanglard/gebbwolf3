...
At the beginning of a frame, the engine switches to the next page and clears the 3D area with ceiling and floor colors. It uses the same trick seen in the 2D renderer and sets the VGA bank mask to 15 to write to all banks simultaneously. With 16-bit registers, 8 pixels can be written with a single instruction. This would result in 5776 instructions for a full 3D canvas. Yet upon looking under the hood, a lesser figure is unveiled.\\ 
\par
...
\par
The Digital Differential Analyzer (a.k.a: DDA) considerably reduces the number of coordinate to search in order to find a contact with a wall. It is not only fast, it is also accurate with no room (pun intended) for errors.
\par
\begin{fancyquotes}
\par
...
\textbf{Kevin Cloud. The Official Hint Manual for Wolfenstein 3D}
 \end{fancyquotes}\\
\par
Be it bug or dedication, a guard on an \cw{AMBUSH} tile will react ONLY to seeing the player. Seeing another actor/dog die right in front of him will not activate him.\\
