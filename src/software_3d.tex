...
\par
But that would be a lot of instructions due mostly to the genericity of the function: Indeed it can accept any height from \cw{0} to \cw{INT\_MAX}. But there is a faster way and it involves the eternal RAM vs speed tradeoff: Pre-generate x86 code specialized to drawing all height of wall from 1 pixels to 200 (the height of the screen). This is done in \cw{BuildCompScale}:\\
\par
\begin{minipage}{\textwidth}
\lstinputlisting[language=C]{code/compscale.c}
\end{minipage}
\par
With this optimization, minifying a 64 textures to a 2 pixel height takes 13 instructions. Minifying to 4 pixels take 25 instructions. It doesn't get any faster than that!
\par
What is the cost of the precompiling these scalers? How much RAM does it use? It takes 1 + 6 instructions per pixel to scale a column. So if all size were generated from 2,4,6...152 (38 pairs of size), we could use Gauss method to calculate the size:\\
\par
$154*38*6=35,112$ bytes\\
\par
...
There is a second level of trick to speed up rendering. The idea is to defer rendering a column of pixels with certain conditions are met. A naive raycaster/compiled scaler coupling would look like this:\\
...
\par
This buffering is done because the renderer allows itself to cheat a little when two rays a deemed similar enough. If rays are similar, they are all drawn to the same height, regardless of their distance from the player point of view. This process introduces a little distortion but unlock a formidable speed up: now the engine can write multiple column on the screen (up to eight) simultaneously.\\
\par
But what makes rays similar? The answer is two rays hitting the same wall and resulting in the same texture coordinate. In short this trick leverage wall magnification:
\begin{figure}[H]
 \centering
 \fullimage{post_optimization_1_show.png}
\end{figure}
In the previous screenshot, the player is so close to the wall that one column of wall texture spread across multiple column on the screen. Normally, because each column on the screen are at a different distance from the POV, there column should be magnified differently.\\
\par
The reason this works find it explanation if the way a frame is stored across the four VGA banks:
 \par
  \begin{minipage}{\textwidth}
...
\centering
 ...
 \end{minipage}
Notice how column representing wall are at the same address in the banks. Leveraging the VGA mask allows to write in multiple banks at the same time and therefore render column of pixels next to each others simultaneously. \\
\par
