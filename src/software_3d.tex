...
The secret of Wolfenstein 3D speed lies in two tricks (described in details later): Precompiled scalers and deferred column rendering.
...
\begin{minipage}{\textwidth}
 ...
 \end{minipage}
\par
Despite being state of the art for 1991, this architecture has a major flaw: Each slice of the game has a different duration which makes the game nondeterministic from one machine to an other. Or even between two runs on the same machine.
\begin{figure}[H]
\centering
 \includegraphics[width=\textwidth]{imgs/drawings/timer/wrong.png}
 ...
 \begin{figure}[H]
\centering
 \includegraphics[width=\textwidth]{imgs/drawings/timer/replay.png}
 \caption{Phase 1: Background}
 \end{figure}
\par
...
\par
 \begin{figure}[H]
\centering
 \includegraphics[width=\textwidth]{imgs/drawings/timer/fixed.png}
 \caption{Phase 1: Background}
 \end{figure}
...
\par
DRAWING: Free objects, free size, no alignment: rays ???\\
\par
But if the word has two constraints:
\begin{itemize}
\item Square axis aligned block: 8 feet tall. TODO: How is the engine translating from imperial to decimal system?
...
The raycasting algorithm is called DDA\footnote{Digital Differential Analysis, commonly used in CG world to draw a line.}. It is a fully handcrafted 740 lines of assembly routine name \cw{PROC AsmRefresh}. Represented in C for readability it consists of two while looks (one checking vertical intersection and one checking the horizontal) ping-ponging with each other via goto. It is highly unorthodox and super efficient:\\
\par
...
\subsubsection{Calculate column heigh}
...
\par
 ...
 It is not very noticeable when a wall is far (as above) but it gets worse when you get closer:
\begin{figure}[H]
\centering
...
\end{figure}
\par
For people who like equations, the overall operation can be seen as the multiplication of a rotation matrix with the vector intercept (dx,dy), discarding the x component:
\begin{figure}[H]
  \centering
  \begin{equation*}
    \scalebox{1.3}{
...
\centering
  \scaledimage{0.8}{fish_eye/fish_eye.png}
\par
\centering
  \scaledimage{0.8}{fish_eye/fish_eye_corrected.png}
...
\centering
  \scaledimage{0.8}{fish_eye/fish_eyed_start_screen2.png}
  \par
\centering
 \scaledimage{0.8}{wolf3d_7_fullframe.png}
\end{minipage}
...
With a distance available for a column, the engine is able to calculate a height for that column. All columns of pixels representing the walls are centered vertically and either magnified or minified. the goal of the exercice is to scale as fast as possible a column of 64 pixels to any height ranging from 2 pixels to max view height (152):\\
\par
 \begin{figure}[H]
\centering
...
Trivia: What as the memory budget of the scaler? JC had to cut corners. What was the cost before cutting corners.\\
\par
\begin{minipage}{\textwidth}
...
\subsubsection{Deferred drawing}
...
barely noticeable. But this cheat allows to draw up to eight column of pixel in three write operations (in practice it is more like two columns
...
\centering
 ...
 \caption{3D Renderer Phase 1: Backed lights} \label{fig:backee_lights}
 \end{figure}


\par
Since wall and sprites are all in the same referential (64x64), it is easy to clip: For each column just compare the height from the occlusion array to the height of the thing:\\
\par
...
TODO: Elaborate on drawing routines which uncompress sprites on the fly.
...
Drawing the weapon at the bottom is very straight forward. It just uses the precompiled scalar with no clipping. Nothing much to say about that :/!\\
...
"Actors" are what make Woflenstein3D exciting. They can be aggressive, they can be sneaky and they can be dumb (like when they are a door). Without them the game would be a beautiful and boring maze. The game designer and developers did a great job.\\
\par
...
\par
What makes enemies interesting to interact with is how the go from standing to being aggressive. They have three ways to detect the player:\\
\begin{itemize}
\item Proximity
\item Vision
\item Sound
