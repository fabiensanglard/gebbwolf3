...
\end{figure}
Notice how blocks are aligned on 4 bytes horizontally but there is no restriction vertically. This is to allow four write/read in one while satisfying the VGA four banks constraints.
...
\end{figure}
\par
A map is 64x64. Since one block is 8 feet, maps can be 512 feet wide and tall.\\
Fixed point variable are used all over the place during this step:\\
calcheigh is using 24:8.\\
position is using 24:8.\\
Angle is an int, [0, 360].\\
...
\centering
  \scaledimage{0.9}{vga_layout/intro.png}
\vspace*{0.5cm}
\centering
  \scaledimage{0.9}{vga_layout/intro_bank.png}
...
The transparency of the sprite is exploited to reduce the storage and RAM consumption. They are stored in a special format skipping transparent section. The routine in charge of drawing decompress a column of sprite while it draws.\\
\par
...
"Actors" can be aggressive, they can be sneaky or they can be dumb (like when they are a door).
\par
All enemies are automaton with four states in their machine:
\begin{itemize}
\item Standing
\item Attack
\item Pursuit
