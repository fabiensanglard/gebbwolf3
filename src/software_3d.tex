...
\end{minipage}
\par
Given a height, \cw{BuildCompScale} generates x86 instructions in the out variable \cw{code}. As a result, instead of having one generic function accepting any height,  Wolfenstein has 256 functions with a hard-coded height. To hard-code the height allows to unroll the loop and reduce the overhead.\\
\par
...
\end{figure}
\par
Testing if a ray is stopped by a door or not costs almost nothing: $A.x + \dfrac{ystep}{2} < doorposition[doorIndex]$. In case of success the ray traverses the wall tile and keeps on being tested against the grid. Otherwise the ray is a hit, the coordinate of the point of interception are calculated as $Intercept.X = A.X + \dfrac{ystep}{2}$ and $Intercept.Y = A.Y + \dfrac{TILE\_SIZE}{2}$ and passed to the renderer.
...
In "path" mode, an agent simulates patrolling. There is no path finding, it is all done via waypoints which change the direction of an agent. After stepping on a "change direction" tile, the agent keeps on walking straight until it hits a wall. All waypoints are placed manually by the map designer with TED5.\\ 
\par
The waypoint system was used in other creative ways like In E1M1 to simulate the agitation of german shepherds (marked a "D"). A detailed view of the right wings of the map from page \pageref{mape1m1} shows no less than eight waypoints making the dogs run in circles and zigzagging.\\
\par
\begin{figure}[H]
 \centering
