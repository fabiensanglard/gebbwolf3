...
\centering
 ...
 \end{figure}
\par
The X coordinate is used to place a sprite horizontally on the screen (the Y coordinate is used for clipping). Sprites don't need to be place vertically for the good reason that sprites and walls textures use the same 64x64 space representing 8 feet by 8 feet.\\
\par
  \begin{minipage}{.5\textwidth}
...
Before drawing each column of a sprite, the engine determines if it is occluded by a wall. This is called clipping and it is an easy step thanks to wall and sprites being in the same 64x64 coordinate system. Once the actor's position is transformed in player space, the distance Y allows to generate a height. That height is the same calculated for the walls. Therefore to find out if a sprite column if behind a wall, the engine just compare the heights of the wall against the height of the actor.\\
\par
...
\end{minipage}
While drawing sprites, the occlusion array is read. If the height of a wall is greater than a sprite, it means the wall is is front of the sprite. The full sprite column rendition is skilled.\\
\par
\begin{minipage}{\textwidth}
...
\begin{enumerate}
 ...
 \end{enumerate}
\par
Transparent texels are not even mentioned. The last command in a column is marked with an offset of \cw{0x00}.\\
\par
...
\par
Because of the format, scalers cannot be used as they are.  

To still take advantage of their speed, the engine patches them with early return instruction \cw{ret} for each commands. In order to to that, the code generator also generates patch locations.\\
\par
\begin{minipage}{\textwidth}
