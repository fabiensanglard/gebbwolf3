...
\par
But sprites are almost never 64 pixels tall. They are usually small and feature transparent parts. Some even have "holes" and are composed of sequences of pixels separated by transparent areas. It is the case of the lamp which for most columns has an upper segment (the lamp) and a lower segment (the shadow).\\
\par
...
\par
There are 64 patch location per scaler (one for each length of pixels besides 64 desired), allowing to write a \cw{ret} instruction causing an early return.\\
\par
...
Drawing the weapon at the bottom is straight forward. It uses the same type of rendition for sprite but with specific code where clipping is disabled.\\ \cw{ScaleShape}
\par
\cw{SimpleScaleShape} is a version of \cw{ScaleShape} with no clipping. Shapes are called compiled shapes.\\
