...
When the player is done setting up the game finally comes time to dive into the 3D engine itself. Something that was kep secret for many years. The 3D engine is based on a simple raycasting technology which is simple yet powerful. The idea is to cast a ray for each column of pixel visible on the screen. Based on the distance d, a height h can be calculated:\\
\par
\begin{figure}[H]
  \centering
  \begin{equation}
...
\end{figure}
\par
Even for a complex scene involving multiple door and rooms, it delivers fast intersection.
\par
\begin{figure}[H]
\centering
...
\begin{figure}[H]
 ...
 \caption{Rendition of the height calculated from the rays distance to the pov.} 
\end{figure}
