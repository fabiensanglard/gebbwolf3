...
The organization of the team was pretty much standard for a game studio of this era. Four guys crammed in one room which dictated a face pace and a strong sense of camaraderie (and a lot of noise disturbance given John Romero and Tom Halls type of interaction). Notice on the upper floor, the SNES where countless games of F-Zero where played and the Donjon \& Dragons area extensively mentioned in "Masters of Doom" book. To have a team member with his apartment directly above the studio was not out of the ordinary\footnote{90 Hours A Week And Loving It!}. The team was setup as follow.
\par
\begin{figure}[H]
  \centering
...
\end{fancyquotes}
\\
You may have noticed in the listing of VGA modes, where \cw{Mode 13h} and \cw{Mode 4h} (figure~\ref{fig:vga_modes} on page~\pageref{fig:vga_modes})  don't have the same starting RAM address. This allows a trick where two graphic cards are plugged in the same PC. One MCA setup as monochrome text mode picking up data at 0xB8000 and a VGA mapped at 0xA0000 running the game normally. This setup allows a developer to debug the game engine with breakpoints.\\
\begin{figure}[H]
\centering
  \shadowbox{
...
All assets were hand drawn with a mouse. Since the VGA stretches the framebuffer when displaying it on the screen (from 320x200 to 320x240), Adrian had to be careful to draw at the same distording resolution as the game would run.\\
\par
\begin{fancyquotes}
...
\begin{itemize}
\item 2D Menus items shipping with the game in \cw{VGAXXXX} archive.
...
\centering
 ...
 \caption{Assets creation pipeline for the 2D Menu items}
\end{figure}
\par
\begin{minipage}{\textwidth}
 ...
 In the engine code, assets usage is "hardcoded" via an enum. This enum is an offset into the 
 ...
 \end{minipage}
\par
\textbf{\underline{Trivia :}} This system led to issues when the source code was released. The \cw{.h} header files provided did not match the assets files from the shareware or early version of Wolfenstein 3D. The header released were from Spears of Destiny. You can see the kind of graphic mess this let to in the article "Let's compile like it is 1994" on \cw{fabiensanglard.net}.\\
...
\par
 \textbf{\underline{Trivia :}} The source code of TED5 was released several years later. Among the source code made of \cw{.C} and \cw{.h} was a mysterious \codeword{\_TOM.PIC}. Converted to PNG it looks like this.\\
\begin{figure}[H]
\centering
...
\par
\note{Sound production pipeline. How were voice recorded ? Who made the german voices: Mein Leben (My life) and Achtung(Attention) coming from?}
...
\end{figure}
\par
\bu{Trivia :}  The YM3812 unmistakable sonority is due to its pecular set of waveform transformer (they right at the output of each oscillator in the drawing). Four waveforms are available on the OPL2.
\par
\begin{figure}[H]
\centering
...
\textbf{\underline{Trivia :}}The engine is very tiny and only uses 94KB. But with 64KB dedicated to the Signon screen (and 768B for the palette), the engine is in fact only 29KB.\\