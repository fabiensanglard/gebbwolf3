...
\par
The CRT Controller scan offset is a 16 bit value which is updated with a little bit of assembly writing to a VGA register: first the high byte and then the low byte. The code would look as follows.\\
\par
\begin{minipage}{\textwidth}
...
\centering
 ...
 \end{figure}
\par
This glitch shows both misalignment and parts of two pages. This problem has to do with atomicity. The CRTC starting address is a 16 bit value but the \cw{out} instruction can only write 8 bits at a time. If the pages are setup one after an other like this.
\par
\begin{figure}[H]
 \centering
