...
\end{minipage}
\par
Since the target machine ran in real mode, the code was compiled using 16 bit instructions only. For operations on \cw{long} (32 bit), Borland used its own math library. In \cw{Patch386} Wolfenstein 3D detects if the CPU is a 386 and replaces Bordland's integral division with instructions using 32 bit registers \cw{eax} and \cw{edx}.\\
\par
\begin{minipage}{\textwidth}
...
\end{minipage}
\par
\cw{PlayLoop} contains the 3D renderer. It is pretty standard with get inputs, update world, and render world approach.\\
\par
\begin{minipage}{\textwidth}
