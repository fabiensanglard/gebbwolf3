...
\end{minipage}
After usage \cw{signon} variable (using 320x200 bytes = 64KB) is unloaded from RAM to make space for the actual game.\\
\par
...
\note{Ask John Carmack and John Romero why they went for Mode-Y instead of Mode-X which offered square pixels.}
 \par
  \begin{minipage}{\textwidth}
...
\par
This speed consideration has a fundamental impact on the engine: To draw anything fast, it has to be drawn \underline{vertically}. So everything in the engine is drawn this way: Walls, Sprites, Menu. Everything, absolutely everything is drawn vertically. The ramification of this hardware constraint are felt everywhere all the way to how assets are stored in RAM (rotated 90 degres). In Wolfenstein 3D: Everything has to be vertical!\\
