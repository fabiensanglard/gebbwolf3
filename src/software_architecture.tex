...
\par
The second issue introduced by Mode-Y is about correctness. With three pages available, the engine draws in page 1, then page 2, then page 3 and then goes back to page 1 (notice there are three buffers but this is not triple-buffering). It never stops. It just instructs the CRT Controller that it should scan the framebuffer at a different offset and keep on running.

To flip between page the CRT Controller is contacted and the new offest from \cw{A000:0000} is entered. The code would look like this:\\
\par
\begin{minipage}{\textwidth}
...
\end{minipage}
\par
Notice how it uses a value of 208 instead of 200 pixels for the height? That doesn't make sense at first sight. I thought it was a typo (after all 0 and 8 are visually close).
\par
