...
\item \codeword{README:} How to build. You can also find a complete tutorial in "Let's compile like its 1992" on \cw{fabiensanglard.net}.
...
\end{itemize}
\par
The program as expected starts in the \cw{int main()} method.\\
\par
\begin{minipage}{\textwidth}
...
\par
\bu{Trivia :} Is there a typo in the filename \cw{AUDIOHEA.WL} ? Correct spelling would be \cw{AUDIOHEAD.WL}. Where is the \cw{D}? This is in fact a limitation of the operating system. DOS only allows 8.3 filename (at most eight characters followed by a dot, then at most three characters for the extension.)
...
\par
This speed consideration has a fundamental impact on the engine: to draw anything fast with the VGA, it has to be drawn vertically. Everything in the engine is drawn this way: walls, sprites, menus. The ramifications of this hardware constraint are felt all the way down to how assets are stored in RAM (rotated 90 degrees) and woven to match the VGA banks layout (this is described in detail in \ref{drawingSprites} "\nameref{drawingSprites}".\\
