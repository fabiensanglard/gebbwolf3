...
\par
\cw{cloc.pl} is a tool which will look at every files in a folder and gather statistics about source code. It helps to get an idea of what to expect.\\
\par
...
\item \codeword{README:} How to build. You can also find a complete tutorial on the author's article "Let's compile like its 1992" on \cw{fabiensanglard.net}.
...
In order to minimize the cost of page miss, the engine preload the page cache before a level start. This is known as the "Get Psyched" screen:
 \par
\begin{figure}[H]
\centering
...
A cache manager is a small but critical component since it loads and decompresses map, graphic and music stored on the filesystem and make them available in RAM. Assets of each kind are stored in two files. A header file contains the offset to allow translation from asset ID to byte offset in the data file.\\
 \par
\begin{figure}[H]
\centering
...
Largely based on Catacomb 3D code and written by Jason Blochowiak. The copy/paste is very visible since 90\% of the functions declared in the header (ID\_US.H) are not actually implemented in \cw{ID\_US.C}. 
It is a poorly named manager since it takes care mostly of text layout. When a \cw{WL\_*} high level routines needs to draw a string, it is passed to \cw{US\_Print} which does all measurement (e.g: Draw string centered)
...
The Sound Manager abstracts interaction with all four sound systems supported: PC Speaker, AdLib, Sound Blaster and SoundSource. It is a beast of its own since it doesn't run inside the engine. Instead it is called via IRQ at a much higher frequency than the engine (the engine runs at a maximum 70Hz, the sound manager ranges from 150Hz to 7000Hz). Being IRQ driven, it must run very fast. It is therefore not only written in assembly, its assets are also privileged when it comes to memory allocations. All of them are loaded in Conventional Memory to avoid a cache miss in the Page Manager.\\
 \par
\begin{figure}[H]
\centering
...
\subsection{Signon}
The first (mild) issue to deal with is the heterogeneous ecosystem of PCs on the market. With different drivers loaded and different sound cards, the engine has to figure out how much RAM is installed and whether it will be able to run. If not it has to let the user know what the problem is. That was important since id Software was a small team and they did not have CSR\footnote{Customer Service Representative.} around to pickup the phone and help unhappy customer troubleshot their issue.\\
\par
...
\par
This speed consideration has a fundamental impact on the engine: To draw anything fast with the VGA, it has to be drawn vertically. Everything in the engine is drawn this way: Walls, Sprites, Menu. Everything, absolutely everything is drawn vertically. The ramifications of this hardware constraint are felt all the way down to how assets are stored in RAM (rotated 90 degrees) and woven to match the VGA banks layout (this is described in details in the "Rendering sprites" section).\\
