...
\\
\bu{Trivia :} Before the times of nmap, WireShark and other ARP poisoning monstruosities, FTP was commonly used to transfer files: It was simple and naively transmitted username and password in clear. Needless to say it did not age well...\\
...
The Hardware chapter describing the VGA left the reader with an unsolved problem: All of the VGA modes lack double buffering capability.\\
\par
 The most appealing mode (13h) offers a single framebuffer at a resolution of 320x200 non-square pixels with 256 indexed colors. In this mode, the VGA circuitry automatically maps the RAM starting at \cw{A000:0000} up to 64K address to the four VGA banks. 
 \par
 \begin{figure}[H]
\centering
...
It is called "chaining" and wastes 75\% of the RAM to offers a linear framebuffer to the developer.\\
...
\centering
 ...
 \par
 The problem really is the Chain-4 chip. And luckily, even though it is undocumented there is a way to disable it. Nobody can remember who discovered  unchaining but it was popularized by Michael Abrash\footnote{in Dr. Dobb's Journal} in July 1991. In his article he described how to turn the VGA into a 320x240 resolution (which features square pixesl) with four buffers and coined it Mode-X.\\
 \par
...
The code sample from the hardware chapter to clear the screen become:\\
\par
\par
\begin{minipage}{\textwidth}
...
\end{minipage}
\par
The code looks innocuous...but as simple as it is, it cannot run at more than 5 frames per second. The problem is that we replaced something done in hardware with something done in software: the \cw{outp} instruction which is very VERY slow.\\
\par
Of course these is a tradeoff to be found and go past this problem. If we look carefully at the screenshot in Figure \ref{fig:vga_layout_for_intro} and Figure \ref{fig:vga_layout_in_3D} on page \pageref{fig:vga_layout_in_3D} there is an interesting property: Horizontal lines of pixels are broken into four areas (and it looks like mashed potatoes) but vertically the column of pixels are still linear and not broken down in banks. Indeed 320 is evenly divisible by 4 so vertically pixels are aligned and grouped in column of 80 pixels. This observation unlock a different algorithm where things are drawn vertically:\\
\par
...
\par
The second issue introduced by Mode-Y is about correctness. With three pages available, the engine draws in page 1, then page 2, then page 3 and then goes back to page 1 (notice there are three buffers but this is not triple-buffering). To flip between page the CRT Controller is contacted and the new offest from \cw{A000:0000} is entered. The code would look like this:\\
\par
\begin{minipage}{\textwidth}
\lstinputlisting[language={[x86masm]Assembler}]{code/pageflip.c}
\end{minipage}
This code looks like it would work. But there is a major flow with it. If you were to run it, every once in a while the screen would show something like this:\\
\par
TODO: Show CRTC mess.\\
\par
The problem has to do with atomicity. Updating the CRTC starting address is 16-bits adress...but it can only be updated with 8-bits write operation. IT is not atomic. If you are unlucky, the CRTC could pickup the scan address just here:\\
\par
\par
\begin{minipage}{\textwidth}
...
\end{minipage}
\par
How do you update atomically a 2 bytes value with 1 byte operation? I will let you ponder that and find the solution in the section dedicated to page flipping.
