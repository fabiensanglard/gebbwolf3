...
The software chapter described how tightly coupled the engine was to the PC hardware. The architecture of the VGA, audio systems, or even a simple trick like the Linear Feedback Shift Register proved sometimes impossible to replicate with machines 100 times more powerful than a 386. This is in contrast to Doom, which was created with portability as a core part of the development process.\\
\par
...
\centering
 ...
 \end{figure}
 \par
The box is as sober as the content of the cartridge. In order to comply with Nintendo's "Game Content Guidelines", id had to drastically alter the graphics of the game and remove blood, dogs\footnote{They were replaced with rats.} and all World War II German era references. Things were also interesting from a technological standing point:\\
\par
...
\centering
 ...
 \end{figure}
 \par
On the screenshot above, the scaling is quite visible. The resolution is 224x192 with the 3D scene tiles scaled 2x from 112x80 to 224x160. The status bar/weapon has a high resolution thanks to non-scaled 32x32 tiles.
...
The console was a mix of powerful components and a severe bottleneck: 
\par
\begin{fancyquotes}
...
\end{figure}
\par
Notice the 4:3 aspect ratio mandated by NTSC standard. The machine gun sprite was not the one from the original Wolf3D, it was borrowed from Doom.\\
...
\subsubsection{iPhone Development Notes}
\par
