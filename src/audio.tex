...
\begin{figure}[H]
	...
	\begin{tabularx}{\textwidth}{ X X  }
	  ...
	  \textbf{I.V.T Entry \#} & \textbf{Type} \\ \bottomrule
		08h & System timer \\
		09h & Keyboard controller \\
		0Ah & Serial port COM2 \\ 
		0Bh & Serial port COM1 \\
		0Ch & Line print terminal 2 \\
		0Dh & Floppy controller \\
		0Eh & Line print terminal 1 \\
		0Fh & RTC timer \\
		11h & Mouse controller \\
		12h & Math co-processor \\
		13h & ATA channel 1 \\
		14h & ATA channel 2  \\
		\bottomrule
	\end{tabularx}
	\caption{The Interrupt Vector Table.}
\end{figure}
Notice \#8 which is associated with the System timer and usually updates the operating system clock. Because IRQ 0 was hijacked, the operating system clock updated by INT 08h could be wrong if the game didn't call the older INT 08 interrupt..\\
\par
...
\item \cw{SDL\_t0ExtremeAsmService} when running at 7000Hz, to play PCM sound in an alternate way. This mode was never enabled in shipping products, see page \pageref{pcs_pcm} for a detailed explanation.
\end{enumerate}
\par
...
\end{minipage}
\par
Notice how \cw{6b} (\cw{10110110}) is sent to the PIC Command register:
\begin{itemize}
...
\begin{figure}[H]
  \centering
  \fullimage{audio/mein_liben.png}
  \caption{Mein Liben PCM 8 bit. You can clearly see the three syllables.}
\end{figure}
\par
...
\begin{figure}[H]
  \centering
  \fullimage{audio/mein_liben_1_bit.png}
  \caption{Mein Liben resampled to 1 bit.}
\end{figure}
\par
The visual representation of a 1 bit per sample "Mein Liben" PCM looks like mashed potatoes but it sounds remarkably good. You can hear it thanks to a passionate YouTuber called Dafe Allen who recompiled the engine with PCM playback enabled\footnote{"Wolfenstein 3D Hack - Digitized PC Speaker Sound Effects": "https://www.youtube.com/watch?v=1BtlsjJRnFU".}.\\
\par
...
\par
The idea is to manipulate the speaker cone manually and instruct it to move faster than it can, interrupting it "somewhere" between it position up or down.\\
\par
