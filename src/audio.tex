...
\end{minipage}
\par
Notice how the \cw{* 60} is not calculated. Once again the engine tries to save as much CPU time as possible by using a bit of RAM. The frequency is read from a lookup table \cw{pcSoundLookup}:\\
\par
\begin{minipage}{\textwidth}
...
\end{minipage}
\par
This codepath shipped but was never enabled. According to John Carmack overhead was too much a problem. Jim Leonard provided a more in depth explanation:\\
...
\end{fancyquotes}
\par
Even if it never ran in production it is still interesting to look inside this function to find out how it manages to fit 8 bit samples into a 1 bit DAC. In the following listing we see that \cw{SDL\_t0ExtremeAsmService} simply takes an 8 bit value and convert it to \cw{00} or \cw{10} to drive bit 1 in I/O port \cw{61h}. When bit 1 is set to 1, the beeper will go in position high and when set to 0, it will go to position low.\\
...
The result sounds surprising good for a 1 bit DAC. If you are curious you can hear it thanks to a passionate YouTuber called Dafe Allen who recompiled the engine with PCM playback enabled\footnote{"Wolfenstein 3D Hack - Digitized PC Speaker Sound Effects": "https://www.youtube.com/watch?v=1BtlsjJRnFU".}.
