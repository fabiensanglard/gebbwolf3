...
\fi    
  ...
  \chapter*{Copyright}
    ...
    \chapter{Software}
       \subfile{src/software_architecture}
       \subfile{src/software_2d}
      \subfile{src/software_3d}
       \subfile{src/audio}
       \subfile{src/inputs}
      \subfile{src/tricks}
       \subfile{src/performances}
...
\chapter{Before Wolfenstein 3D}
        \subfile{src/softdisk}
    
    \chapter{XMS vs EMS} 
       \subfile{src/xms_vs_ems}   
    \chapter{The 640KB Barrier}\label{chap:barrier640}
        \subfile{src/mem_barrier}
    \chapter{CONFIG.SYS and AUTOEXEC.BAT}
         \subfile{src/config_autoexec}
    \chapter{Good Stuff}
          \subfile{src/good_stuff}
